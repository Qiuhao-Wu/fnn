...
with tf.device('/gpu:0'):
        ...
        for epoch in tqdm(range(train_epochs)):
            ...
            #test
            loss_test,acc_test = session.run([loss,accuracy],feed_dict={data_x: mnist.test.images.reshape((-1,size,size)), data_y: mnist.test.labels})
            test_loss_list.append(loss_test)
            test_acc_list.append(acc_test)
            print("epoch :{} test_loss:{:.4f} test_acc:{:.4f}".format(epoch+1,loss_test,acc_test))
...
fig,ax1=plt.subplots()
ax2=ax1.twinx()
Ins3=ax1.plot(np.arange(50),test_loss_list,label='Test_Loss')
Ins4=ax2.plot(np.arange(50),test_acc_list,'r',label='Test_Accuracy')
ax1.set_xlabel('interation')
ax1.set_ylabel('testing loss')
ax2.set_ylabel('testng accuracy')

Ins_=Ins3+Ins4
labels=['Loss','Accuracy']
plt.legend(Ins_,labels,loc='best')
plt.show()
