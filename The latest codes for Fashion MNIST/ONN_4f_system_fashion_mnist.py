...
train_label = train_labels[:55000]
validation_label =train_labels[55000:]
...
with tf.device('/gpu:0'):
    ...
    for epoch in tqdm(range(train_epochs)):
        ...
        #test
        loss_validation,acc_validation = session.run([loss,accuracy],feed_dict={data_x:validation_image.reshape((-1,size,size)), data_y: validation_label})
        validation_loss_list.append(loss_validation)
        validation_acc_list.append(acc_validation)
        print("epoch :{} test_loss:{:.4f} test_acc:{:.4f}".format(epoch+1,loss_validation,acc_validation))
